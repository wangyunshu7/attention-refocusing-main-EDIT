...
from chatGPT import read_txt_hrs, load_gt, load_box, save_img, read_csv, generate_box_gpt4, Pharse2idx_2, \
    process_box_phrase, format_box, draw_box_2
import torchvision.transforms as transforms
...
def run(meta, models, info_files, p, starting_noise=None, iter_id=0, img_id=0, save=True):
    ...
    samples_fake = sampler.sample(S=steps, shape=shape, input=input, uc=uc, guidance_scale=config.guidance_scale,
                                  mask=inpainting_mask, x0=z0, loss_type=args.loss_type)
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loss_type", choices=['standard', 'SAR', 'CAR', 'SAR_CAR'], default='SAR_CAR',
                        help='Choose one option among the four options for what types of losses ')
