...
def load_ckpt(ckpt_path):
    ...
    # config['text_encoder'] = "/root/.cache/huggingface/hub/models--openai--clip-vit-large-patch14/snapshots/32bd64288804d66eefd0ccbe215aa642df71cc41/model.safetensors"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", choices=['counting','spatial','color','size'], default = "spatial")
    parser.add_argument("--box_pickle", type=str, default=None)
    parser.add_argument("--file_save", type=str, default="./output")
    parser.add_argument("--data_type", choices=['HRS','Drawbench','compbech'], default='compbech')
    parser.add_argument("--loss_type", choices=['standard','SAR','CAR','SAR_CAR','OURS'],default='SAR_CAR', help='Choose one option among the four options for what types of losses ')
    ...
    elif args.data_type=='compbech':
        prompts = []
        with open("./T2I-CompBench_dataset/color_val.txt","r") as f:
            lines = f.readlines()
            for line in lines:
                prompts.append(line.replace("\n", "").replace(".", "").strip())
        all_caption_layout = {}
        with open("./layout_generation/color_val.txt", "r") as f:
            lines = f.readlines()
    ...
    for i, prompt in enumerate(prompts):
        ...
        for meta in meta_list:
            ...
            meta["prompt"] = prompt
            text = prompt
            if prompt in all_caption_layout.keys() :
                p_layout = all_caption_layout[prompt]
                o_names = []
                o_boxes = []
                for name, box in p_layout.items():
                    o_names.append(name.split("-")[0])
                    o_boxes.append([x * 2 for x in box])
